...
import HunterAlpha.modules.sql.global_bans_sql as gban_sql
import HunterAlpha.modules.sql.users_sql as user_sql
from HunterAlpha import DEV_USERS, OWNER_ID, dispatcher
...
def get_invalid_chats(update: Update, context: CallbackContext, remove: bool = False):
    ...
    for chat in chats:
        ...
        if ((100 * chats.index(chat)) / len(chats)) > progress:
            progress_bar = f"{progress}% completado en la obtención de chats inválidos."
            if progress_message:
                try:
                    bot.editMessageText(
...
def dbcleanup(update: Update, context: CallbackContext):
    ...
    msg.reply_text("Obteniendo un recuento de chat no válido ...")
    invalid_chat_count = get_invalid_chats(update, context)

    msg.reply_text("Obtener un recuento de gbanned no válido ...")
    invalid_gban_count = get_invalid_gban(update, context)

    reply = f"Total de chats no válidos - {invalid_chat_count}\n"
    reply += f"Total de usuarios de gbanned no válidos - {invalid_gban_count}"

    buttons = [[InlineKeyboardButton("Limpieza de Base datos ", callback_data="db_cleanup")]]
...
def callback_button(update: Update, context: CallbackContext):
    ...
    if query_type == "db_leave_chat":
        if query.from_user.id in admin_list:
            ...
            query.answer("No se le permite usar esto.")
    elif query_type == "db_cleanup":
        if query.from_user.id in admin_list:
            bot.editMessageText("Limpieza de Base datos ...", chat_id, message.message_id)
            invalid_chat_count = get_invalid_chats(update, context, True)
            invalid_gban_count = get_invalid_gban(update, context, True)
            reply = "Limpiado {} chats y {} usuarios gbanned de db.".format(
...
__mod_name__ = "Limpieza de base de datos"
